...
export default function UserList() {
  ...
  return (
    <div className="p-4">
      ...
      <table className="min-w-full border border-gray-300">
        ...
        <tbody>
          {users.map((user) => (
            <tr
              key={user.id}
              className="even:bg-sky-100 odd:bg-sky-200 transition-colors"
            >
              <td className="border px-4 py-2 text-center">{user.id}</td>
              <td className="border px-4 py-2">{user.name}</td>
              <td className="border px-4 py-2">{user.email}</td>
      ...
      {/* Scroll Loader or End Message */}
      {hasMore ? (
        <div
          ref={observerRef}
          className="h-12 mt-40 flex items-center justify-center text-lg text-black bg-amber-200 rounded shadow"
        >
          Scroll down to load more...
        </div>
      ) : (
        <div className="h-12 mt-40 flex items-center justify-center text-lg text-green-800 bg-green-300 rounded shadow">
          ✅ All users loaded.
        </div>
      )}
    </div>
